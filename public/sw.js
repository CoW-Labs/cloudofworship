...
const VERSION_ENDPOINT = 'https://api.cloudofworship.com/api/v1/app-config/info';
...
self.addEventListener('activate', (event) => {
  event.waitUntil((async () => {
    ...
    if (!latestVersion || latestVersion !== prevVersion) {
      // Delete all caches
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.origin === self.location.origin && !url.pathname.endsWith('/api/v1/app-config/info')) {
    event.respondWith((async () => {
      const appVersion = await getVersion() || 'v0';
